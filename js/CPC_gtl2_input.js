...
//被保險人生日_逐筆
    $('#InsurBthStep').click(function(){
        $('#BirthADSTEP').attr("checked", true);
    });
    
    $('#InsurBthTWStep').click(function(){
        $('#BirthTWSTEP').attr("checked", true);
    });    
//END  

    $( "#InsurBthStep, #InsurBthSim" ).datepicker({
        defaultDate: '-30y',
        maxDate: 0,
        minDate: '-80y',
        yearRange: 'c-50:c+30',
    ...
    $('#enTypeY').click(function(){
        $('#TypeStep').show();
        $('#TypeSing').hide();
    });
    
    $('#enTypeN').click(function(){
        $('#TypeSing').show();
        $('#TypeStep').hide();
    });    
...
//            }
    ...
    $('#15PRJBtnSim').click(function(){//新增資料明細_簡易輸入
       $('#actn_SumitIns').attr('disabled', false);
       $('#InsList').empty();
       var method =$("input[name='enType']:checked").val(); //radio 取值，注意寫法
       if( typeof(method) == "undefined"){ // 注意檢查完全沒有選取的寫法，這行是精華
           alert( "請選取操作方式！");
           return false;
       }else if(method=='N'){
    
        var $InsList = $('#InsList2');
        var $InsurNm=$('#InsurNmSim').val();
        var $InsurID=$('#InsurIDSim').val();
       var $InsurBth;
       if($('input[name=BirthY]:checked').val()=="TW"){
           $InsurBth=$('#custBthTW').val();
       }else{
           $InsurBth=$('#InsurBthSim').val();
       }        
        var $InsurPS=$('#InsurPeopleSim').val();
        //var $InsurPP=$('#InsurPPSim').val();
        var $ADDInsAmt=$('#ADDInsAmtSim').val();
        var $MRInsAmt=$('#MRInsAmtSim').val();
        var $HARInsAmt=$('#HARInsAmtSim').val();
        var $beneficiary=$('#beneficiarySim').val();
        var $beneficiaryRe=$('#beneficiaryReSim option:selected').text();
        ...
        $TR1.append('<td class="tableSubhead" width="20%" colspan="2">人數</td>');
        //$TR1.append('<td class="tableSubhead" width="20%">護照號碼</td>');
        //$TR.append('<td class="tableData" colspan="4"><s:text name="label.input.step"/></td>');
        $TB.append($TR1);
            
        $TR2.append('<td class="tableData" width="15%"><input id="InsurNm" name="appM.InsurNm"  value='+$InsurNm+'></td>');
        $TR2.append('<td class="tableData" width="20%"><input id="InsurID" name="appM.InsurID"  value='+$InsurID+'></td>');
        $TR2.append('<td class="tableData" width="20%"><input id="InsurBth" name="appM.InsurBth"  value='+$InsurBth+'></td>');
        $TR2.append('<td class="tableData" colspan="2" width="20%">等<input id="InsurPP" name="appM.InsurPP"  value='+$InsurPS+'>人</td>'); 
        $TB.append($TR2);
        ...
        $TR4.append('<td class="tableData" width="15%"><select id="ADDInsAmtList" name="applicant.MRInsAmtList" ><option>'+$MRInsAmt+'</option></select>萬元</td>');
        $TR4.append('<td class="tableData" width="15%"><select id="ADDInsAmtList" name="applicant.HARInsAmtList" ><option>'+$HARInsAmt+'</option></select>萬元</td>');
        $TR4.append('<td class="tableData" width="20%"><input id="beneficiary" name="appM.beneficiary" value='+$beneficiary+'></td>');
        $TR4.append('<td class="tableData" width="15%"><select id="ADDInsAmtList" name="applicant.HARInsAmtList" ><option>'+$beneficiaryRe+'</option></select></td>');        
        $TB.append($TR4);        
    ...
    $('#15PRJBtnA').click(function(){//新增資料明細_逐筆輸入
       ...
       if( typeof(method) == "undefined"){ // 注意檢查完全沒有選取的寫法，這行是精華
        ...
        var $InsurBth;
       if($('input[name=BirthSTEP]:checked').val()=="TW"){
           $InsurBth=$('#InsurBthTWStep').val();
       }else{
           $InsurBth=$('#InsurBthStep').val();
       }         
        ...
        $TR2.append('<td class="tableData" width="20%"><input id="InsurID" name="appM.InsurID"  value='+$InsurID+'></td>');
        $TR2.append('<td class="tableData" width="20%"><input id="InsurBth" name="appM.InsurBth"  value='+$InsurBth+'></td>');
        $TR2.append('<td class="tableData" width="20%"><input id="InsurENm" name="appM.InsurENm"  value='+$InsurENm+'></td>');
        $TR2.append('<td class="tableData" width="20%"><input id="InsurPP" name="appM.InsurPP"  value='+$InsurPP+'></td>');      
        $TB.append($TR2);
        ...
        $TR4.append('<td class="tableData" width="15%"><select id="ADDInsAmtList" name="applicant.HARInsAmtList" ><option>'+$HARInsAmt+'</option></select>萬元</td>');
        $TR4.append('<td class="tableData" width="20%"><input id="beneficiary" name="appM.beneficiary" value='+$beneficiary+'></td>');   
        $TR4.append('<td class="tableData" width="15%"><select id="ADDInsAmtList" name="applicant.HARInsAmtList" ><option>'+$beneficiaryRe+'</option></select><input class="button36big" type="button" value="更多"/> </td>');        
        $TB.append($TR4);        
